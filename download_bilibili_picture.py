...
def get_dynamic_images(mid, next_offset=0, limit=20):
    """
    获取某个 up 主的动态图片
    ...
    # 构造请求头，模拟浏览器访问
    url = f'https://api.vc.bilibili.com/dynamic_svr/v1/dynamic_svr/space_history?host_uid={mid}&offset_dynamic_id={next_offset}&need_top=0&limit={limit}'
    ...
    for item in items:  # 遍历动态列表
        item = json.loads(item.get('card')).get(
            'item')
        if not item:
            continue
